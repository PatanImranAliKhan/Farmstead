...
@Component({
  selector: 'app-agricommerce',
  templateUrl: './agricommerce.component.html',
  styleUrls: ['./agricommerce.component.css']
})
export class AgricommerceComponent implements OnInit , OnDestroy {
  ...
  ngOnInit(){
    ...
    this.requirements.getAllRequirements("agrirequirements")
  ...
  Getall()
  {
    this.invalid=false;
    this.requirements.getAllRequirements("agrirequirements")
    .subscribe((details) => { this.products=details;this.errMess=null; this.invalid=false;}, (errMess) => {this.errMess=errMess;this.products=null; this.invalid=true;this.Error()});
    this.data="";
    if(this.products.length==0)
    {
      this.invalid=true;
